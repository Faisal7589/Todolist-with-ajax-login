    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(check === false) {
                ...
                //create a new user
                connection.query("INSERT INTO loginajax.users set ? ", data, function(error, rows) {
...
return new Promise((resolve, reject) => {
    try{
        connection.query("SELECT * from loginajax.users where email = ?", email, function(error, rows) {
            if(error) reject(error);
